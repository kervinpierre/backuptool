...
public class JobSetStateWriterTest
{
    ...
    {
        Path backupDir = Paths.get("/tmp/backupTest/tmp/JobSetStateWriterTest/A001_testWrite/");
        ...
        Path outPath = backupDir.resolve("jobSetStateWrite_A001.xml");

        log.debug(String.format("Writing to '%s'", outPath));

        Files.deleteIfExists(outPath);

        JobSetState currJobSet = JobSetState.from(UUID.randomUUID(),
                null,
                null,
        ...
        currJobSet.getJobs().put(currJobState.getJobId(), currJobState);

        JobSetStateWriter.write(currJobSet, outPath);
